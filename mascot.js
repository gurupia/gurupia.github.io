...
class Mascot {
    ...
    setupSettings() {
        ...
        const savedBGMData = localStorage.getItem('mascot-bgm-data');

        if (bgmAudio) {
            if (savedBGMData) {
                bgmAudio.src = savedBGMData;
            }
            bgmAudio.volume = 0.5;

            if (savedBGM && bgmToggle) {
        ...
        // Handle BGM Upload
        if (bgmUpload && bgmAudio) {
            bgmUpload.addEventListener('change', (e) => {
                const file = e.target.files[0];
                if (file) {
                    // Check size (limit to ~3MB for localStorage)
                    if (file.size > 3 * 1024 * 1024) {
                        alert("File is too large! Please use a file smaller than 3MB.");
                        return;
                    }
                    ...
                    reader.onload = (event) => {
                        ...
                        // Save to localStorage
                        try {
                            localStorage.setItem('mascot-bgm-data', result);
                        } catch (e) {
                            alert("Storage full! Could not save BGM for next visit.");
